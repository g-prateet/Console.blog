    ...
    <div className="max-w-6xl p-8 mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-10 items-center justify-between">
        <div className="relative w-full md:w-96">
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white/60 dark:bg-[#2A2E2C]/60 border border-[#A599B5]/40 dark:border-[#A599B5]/20 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#007EA7]/40 transition-all text-[#56494C] dark:text-[#EAE7E1] dark:placeholder-[#EAE7E1]/40"
          />
          <span className="absolute left-3 top-3 text-[#A599B5]">
        ...
        {selectedTag && (
          <div className="flex items-center gap-3 px-4 py-2 bg-[#A599B5]/15 dark:bg-[#A599B5]/20 border border-[#A599B5]/30 rounded-full">
            <span className="text-[#56494C] dark:text-[#EAE7E1] text-sm font-medium">
              Tag: <span className="font-bold text-[#5B7553] dark:text-[#7EA873] font-mono">#{selectedTag}</span>
            </span>
            <button
              onClick={() => setSelectedTag(null)}
              className="text-[#56494C]/60 dark:text-[#EAE7E1]/60 hover:text-[#56494C] dark:hover:text-[#EAE7E1] transition-colors"
      ...
      {loading ? (
        ...
        <div className="py-20 text-center flex flex-col items-center justify-center bg-white/30 dark:bg-[#2A2E2C]/30 rounded-3xl border border-[#A599B5]/20 border-dashed">
          ...
          <h3 className="text-2xl font-serif font-bold text-[#56494C] dark:text-[#EAE7E1] mb-2">No articles yet</h3>
          <p className="text-[#56494C]/70 dark:text-[#EAE7E1]/70">Be the first to write one and share your knowledge.</p>
        </div>
      ) : filteredPosts.length === 0 ? (
        <div className="py-20 text-center">
          <p className="text-[#56494C]/70 dark:text-[#EAE7E1]/70 text-lg">No articles match your search criteria.</p>
        </div>
      ) : (
        /* Article Grid */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                {post.tags && post.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-[#A599B5]/10 dark:border-[#A599B5]/20">
                    {post.tags.map((tagObj, index) => (
                      <button
                        key={index}
                        onClick={(e) => { e.stopPropagation(); setSelectedTag(tagObj.name); }}
